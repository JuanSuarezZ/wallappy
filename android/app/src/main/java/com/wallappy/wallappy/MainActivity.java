    ...
    private void downloadImageNew(String filename, String downloadUrlOfImage){
        try{
            ...
            request.setAllowedNetworkTypes(DownloadManager.Request.NETWORK_WIFI | DownloadManager.Request.NETWORK_MOBILE)
                    ...
                    .setDestinationInExternalPublicDir(Environment.DIRECTORY_PICTURES,File.separator + "Wallpaper images"+ File.separator + filename + ".jpg");
            dm.enqueue(request);
            
            MainActivity.res.success(true);
            // Toast.makeText(this, "Imagen descargada :)", Toast.LENGTH_SHORT).show();
        }catch (Exception e){
            
            // Toast.makeText(this, "no se pudo descargar la imagen :)" + e.getMessage().toString(), Toast.LENGTH_SHORT).show();
        }
    }
    ...
    protected final Boolean doInBackground(Pair<Bitmap, String>... pairs) {
        switch (pairs[0].second) {
            ...
            case "3": {
                WallpaperManager wallpaperManager = WallpaperManager.getInstance(mContext);
                try {
                    // Log.i("Arguments ","version: " + Build.VERSION.SDK_INT);
                    // Log.i("Arguments ","version2: " + Build.VERSION_CODES.N);
                    // if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
